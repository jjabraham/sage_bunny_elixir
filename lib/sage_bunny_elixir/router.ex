defmodule SageBunnyElixir.Router do
  alias SageBunnyElixir.JsonUtils, as: JSON
  use Plug.Router
  ...
  post "/test1" do
    send_resp(conn, 200, "TEST1")
  end

  get "/knockknock" do
    case Mongo.command(:mongo, ping: 1) do
      {:ok, _res} -> send_resp(conn, 200, "Who's there?")
     {:error} -> send_resp(conn, 500, "watchu talkin bout, Willis?")
    end
  end
  ...
  post "/post" do
    case conn.body_params do
      ...
      _ -> send_resp(conn, 400, "invalid input")
    end
  end
  ...
  get "/post/:id" do
    ...
    case doc do
      ...
      %{} ->
        post =JSON.normaliseMongoId(doc)
          |>Jason.encode!()
